...
router.post('/obj', (req, res) => {
	if(typeof req.body.Id != 'string' || !req.body.Id){
		...
		if(typeof req.body.Mdp != 'string' || !req.body.Mdp){
			throw new Error("MdpLogin non défini")
		}
	bcrypt.hash(req.body.Mdp, 8, function(err, hash) { ///ajout utilisators
...
router.post('/addpd', (req, res) => { //ajout de produit
...
router.post('/del2', (req, res) => { // delete produit
	for (var i = 0; i < obj2.table.length; i++) {
		  var elem = Object.values(obj2.table[i]);;
		  console.log("BCRYPT TEST : " + elem[0] +" 2: " +elem[1]);
...
router.post('/upd', (req, res) => { //update mdp user
	if(typeof req.body.MdpUpd != 'string' || !req.body.MdpUpd){
			throw new Error("MdpLogin non défini")
		}
	for (var i = 0; i < obj.table.length; i++) {
		  var elem = Object.values(obj.table[i]);
...
router.post('/editPd', (req, res) => { //edit produit
	for (var i = 0; i < obj2.table.length; i++) {
		  var elem = Object.values(obj2.table[i]);
